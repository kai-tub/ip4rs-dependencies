...
RUN echo "mamba activate ${conda_env}" >> "${HOME}/.bashrc"

# these are used in ENTRYPOINT `start-notebook.sh` script
ENV NOTEBOOK_ARGS="--no-browser --ip=0.0.0.0 --ServerApp.trust_xheaders=True --ServerApp.disable_check_xsrf=False --ServerApp.allow_remote_access=True --ServerApp.allow_origin='*'"
